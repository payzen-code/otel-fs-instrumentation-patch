...
const createHook = sinon.spy((_functionName) => {
    throw createHookError;
});
const endHookError = new Error('endHook failed');
const endHook = sinon.spy((_functionName) => {
    throw endHookError;
});
const pluginConfig = {
    createHook,
...
const assertNotHookError = (err) => {
    assert.ok(err &&
        err.message !== createHookError.message &&
        err.message !== endHookError.message, 'Hook error shadowed the error from the original call');
};
const assertSuccessfulCallHooks = (expectedFunctionName) => {
    ...
    assert(!endHookCall.getCall(0).args[1].error, 'Did not expect an error');
};
const assertFailingCallHooks = (expectedFunctionName) => {
    const createHookCall = createHook.withArgs(expectedFunctionName);
    sinon.assert.called(createHookCall);
    sinon.assert.threw(createHookCall, createHookError);
    const endHookCall = endHook.withArgs(expectedFunctionName);
    sinon.assert.called(endHookCall);
    sinon.assert.threw(endHookCall, endHookError);
    assert(endHookCall.getCall(0).args[1].error, 'Expected an error');
};
